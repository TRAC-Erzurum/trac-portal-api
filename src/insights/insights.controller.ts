...
export class InsightsController {
  ...
  async getPersonalNetStats(
    ...
    return this.insightsService.getPersonalNetStats(user.id, branchFilter ?? 'all', branchId)
  }
  ...
  async getCommunityStats(
    ...
    return this.insightsService.getCommunityStats(user?.id, branchFilter ?? 'all', branchId, p)
  }
  ...
  async getTopStreak(
    ...
    const branchIds = await this.insightsService.resolveBranchIdsForScope(user?.id, branchFilter ?? 'all', branchId)
  ...
  async getPersonalTrend(
    ...
    return this.insightsService.getPersonalTrend(user.id, branchFilter ?? 'all', branchId)
  }
  ...
  async getMonthlyTrend(
    ...
    const branchIds = await this.insightsService.resolveBranchIdsForScope(user?.id, branchFilter ?? 'all', branchId)
    return this.insightsService.getMonthlyTrend(Math.min(Math.max(months, 1), 24), branchIds)
  }
  ...
  async getNetsAttendeesTrend(
    ...
    const branchIds = await this.insightsService.resolveBranchIdsForScope(user?.id, branchFilter ?? 'all', branchId)
    return this.insightsService.getNetsAttendeesTrend(Math.min(Math.max(limit, 1), 50), branchIds)
  }
